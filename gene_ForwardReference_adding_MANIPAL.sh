...
#	2>	After Downloading data from UCSC Table Browser, name the files with the correct name of the GENES [named in "GENENAME.fasta" format].
...
sed -i.bak '1 s/^.*$/>'$chr':'$position'/' $gene_name.fasta 

mv $gene_name.fasta LGdb/
...
while true; do
    ...
    case $yn in
        [Yy]* ) bash gene_ForwardReference_adding_MANIPAL.sh; break;;
